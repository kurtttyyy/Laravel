        <form id="employee-leave-form" method="POST" action="{{ url('') }}" class="space-y-6">
            @csrf

            <!-- LEAVE APPLICATION FORM -->
            <div id="employee-leave-form-print-area" class="border-2 border-black p-6 rounded-lg space-y-4">
            ...
            <div class="flex justify-end">
                <button
                    type="button"
                    onclick="downloadEmployeeLeaveForm()"
...
<script>
    function downloadEmployeeLeaveForm() {
        const form = document.getElementById('employee-leave-form');
        const printableArea = document.getElementById('employee-leave-form-print-area');

        if (!form || !printableArea || typeof html2pdf === 'undefined') {
            return;
        }
        ...
        const options = {
            margin: 8,
            filename: `employee-leave-form-${fileDate}.pdf`,
            image: { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2, useCORS: true },
            jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
        };

        html2pdf().set(options).from(printableArea).save();
    }
</script>
